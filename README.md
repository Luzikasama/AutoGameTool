...
- **循环执行**：整图按轮次循环
- **悬浮框**：可开关的置顶小窗，显示循环进度与当前节点，并内置 **启动/停止、录制、循环次数 ±** 三个按钮，以及右上角的**回到界面图标**（可拖拽、点 ✕ 收起、**不抢焦点**）
...
- **顶栏显示版本号**：右上角 `v0.7.0`，用来确认当前页面/进程是不是最新版（排查「界面里没有某功能」时最省事）
...
│  ├─ test_engine_state.py    # 快捷键触发锁 / 悬浮框状态去重（纯逻辑，不起服务）
│  ├─ test_state_sync.py      # 端到端：WebUI 与悬浮框启停状态一致、停止真能停
...
| `state` | `{state: "running"\|"idle"}` | 运行状态（引擎为唯一事实来源，每秒兜底广播） |
...
三个脚本合计 **49 条断言**，覆盖的都是「改错了很难靠肉眼发现」的状态逻辑：

- `test_engine_state.py`（23 条）：按住 `alt` 连按两次 `f1` 必须触发两次；长按 `f1` 只触发一次；悬浮框状态去重（值没变不重绘）；`overlay` 模块级 API 完整性
- `test_state_sync.py`（19 条）：`/run` 之后 `/run/state` 与 `/overlay/state` 必须同时为「运行中」；`/run/stop` 后两边都回到「未运行」；长延时能被立刻中断；反复停止幂等；状态没被卡死
