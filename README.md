...
| 输入文本 | 📝 | 输入任意文本 |
| 键鼠录制 | ⏺ | **alt+9** 开始/停止，录制鼠标(点击/滚轮)与键盘(按下/抬起)，打包为**一个步骤**，支持 0.25x~4x 变速回放；可**一键拆分为可编辑节点**（见 [10.12](#1012-录制模型与一键拆分v060-起v061-补顶栏入口)） |
...
- **录制可拆分**：录完的「键鼠录制」步骤，点**顶栏常驻**的 **✂ 拆分录制**（或选中该节点后在右侧属性面板点 **✂ 拆分为可编辑步骤**），自动变成 点击 / 按键 / 延时 / 滚轮 等独立节点，可单独改坐标、改键、调顺序（见 [10.12](#1012-录制模型与一键拆分v060-起v061-补顶栏入口)）
- **顶栏显示版本号**：右上角 `v0.6.1`，用来确认当前页面/进程是不是最新版（排查「界面里没有某功能」时最省事）
...
### 10.12 录制模型与一键拆分（v0.6.0 起，v0.6.1 补顶栏入口）
...
| 录制的宏里没有鼠标移动轨迹了 | v0.6.0 起的**有意变更**：轨迹点既无法编辑又拖慢回放，改为只记点击坐标（回放时直接落到该点）。旧脚本的轨迹事件仍可正常回放，见 [10.12](#1012-录制模型与一键拆分v060-起v061-补顶栏入口) |
...
5. **编辑器没有撤销 / 重做**：拆分（顶栏 **✂ 拆分录制** 与属性面板 **✂ 拆分为可编辑步骤** 都一样）会**就地替换**原录制节点且不可撤销（保存前可重新加载工程放弃改动）。拆分前建议先保存一份 `.agflow`。
