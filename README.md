...
- **顶栏显示版本号**：右上角 `v0.6.2`，用来确认当前页面/进程是不是最新版（排查「界面里没有某功能」时最省事）
...
> ⚠️ 拆分与打包都是**就地替换**且**不可撤销**（编辑器暂无撤销/重做）。建议操作前先保存一份 `.agflow`；若还没另存覆盖，重新加载原工程即可放弃改动。
...
5. **编辑器没有撤销 / 重做**：拆分与打包（顶栏 **✂ 拆分录制** / **📦 打包合并**，以及属性面板的 **✂ 拆分为可编辑步骤**）都会**就地替换**原有节点且不可撤销（保存前可重新加载工程放弃改动）。操作前建议先保存一份 `.agflow`。
