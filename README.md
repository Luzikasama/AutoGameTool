...
- **循环执行**：整图按轮次循环
- **悬浮框**：可开关的置顶小窗，显示循环进度与当前节点，并内置 **启动/停止、暂停/继续、录制、循环次数 ±** 按钮，以及右上角的**回到界面图标**（可拖拽、点 ✕ 收起、**不抢焦点**）
...
| 路由 | **Vue Router** | ^4.6.4 | 打开即进编辑器（`/` 重定向到 `/editor`；旧首页保留在 `/home`） |
...
│  │  ├─ components/SettingsModal.vue  # 设定面板（选图 → 截取 → 透明度）
│  │  ├─ views/
│  │  │  ├─ Home.vue          # 首页
...
三个脚本合计 **80 条断言**，覆盖的都是「改错了很难靠肉眼发现」的状态逻辑：

- `test_engine_state.py`（31 条）：按住 `alt` 连按两次 `f1` 必须触发两次；长按 `f1` 只触发一次；悬浮框状态去重（值没变不重绘）；`overlay` 模块级 API 完整性；Executor 暂停状态机（空闲暂停无效、停止清暂停、任务结束后 `reconcile` 复位）
...
> 两个端到端脚本会真起一个引擎进程，并把 `APPDATA` 指向项目内 `.tmp\`，因此**不会碰你自己的 `%APPDATA%\AutoGameTool` 配置**（顺带保证测试期间悬浮框是关的、不弹窗）。它们需要 8765 端口空闲：先退出正在运行的 AutoGameTool。
...
| **↗▣（右上角图标）** | 把 WebUI 所在**浏览器**窗口恢复并切到前台（标题匹配 + 浏览器识别，最小化状态也能唤回；同名文件夹的资源管理器窗口不会被误选） |
...
| 悬浮框点右上角图标没反应 | 只匹配**浏览器**窗口，且标题需含 `AutoGameTool`：确认标签页还开着；若该标签页没被激活（标题不含它），先手动切一下。日志面板会给出提示。鼠标移上去图标会变成青色，说明可点击 |
...
| 打开页面直接就是编辑器，找不到首页了 | v0.7.1 起的**有意变更**：`/` 直接跳到 `/editor`。旧首页仍在 `/home`；「新建脚本」挪到编辑器右上角「保存」左边 |
