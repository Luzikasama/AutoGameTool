...
这个项目完全由 **DeepSeek Harness** 驱动完成，使用的模型是 **DeepSeek V4 Flash**。

从需求梳理、技术选型、前后端编码，到打包脚本、安装包和这份文档，全部在 Harness 里由 AI 智能体协作产出；人的工作只有三件：说清需求、验收效果、在关键分叉上拍板。仓库里没有预先准备的代码骨架，也没有事后手工补写的业务代码。

把它开源，是想留一个可复现的样本：一个真实可用的 Windows 桌面工具（Vue 3 + Python/FastAPI + OpenCV + NSIS 打包），从零做到安装包的全过程都由 AI 完成，会是什么样子、又会踩哪些坑。如果你也在做类似的事，欢迎参考，也欢迎提 Issue 交流。
